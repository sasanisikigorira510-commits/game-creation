...
let entries: [MonsterEntry] = [
    MonsterEntry(raceFolder: "ドラゴン", sourceFolder: "ドラゴン1", resourceFolder: "Dragon", outputKey: "dragon_whelp", monsterId: "monster_dragon_whelp", displayName: "ヒナドラ", raceId: "dragon", classRank: 1, encyclopediaNumber: 1, element: 3, rangeType: 1, damageType: 1, maxHp: 58, attack: 13, magicAttack: 12, defense: 9, magicDefense: 8, attackSpeed: "1.08", attackRange: "2.80", normalAttackTargetCount: 1, description: "炎の息を覚え始めた幼いドラゴン。素早く動き、序盤の万能アタッカーとして扱いやすい。", fusionExclusive: 0),
    MonsterEntry(raceFolder: "ドラゴン", sourceFolder: "ドラゴン2", resourceFolder: "Dragon", outputKey: "flare_drake", monsterId: "monster_flare_drake", displayName: "フレアドレイク", raceId: "dragon", classRank: 2, encyclopediaNumber: 2, element: 3, rangeType: 1, damageType: 1, maxHp: 86, attack: 21, magicAttack: 23, defense: 15, magicDefense: 14, attackSpeed: "0.98", attackRange: "3.05", normalAttackTargetCount: 1, description: "火炎をまとって飛ぶ成長期の竜。中距離からブレスで敵を焼き払う。", fusionExclusive: 0),
    MonsterEntry(raceFolder: "ドラゴン", sourceFolder: "ドラゴン3", resourceFolder: "Dragon", outputKey: "abyss_dragon", monsterId: "monster_abyss_dragon", displayName: "蒼黒竜アビス", raceId: "dragon", classRank: 3, encyclopediaNumber: 3, element: 2, rangeType: 1, damageType: 1, maxHp: 126, attack: 30, magicAttack: 38, defense: 24, magicDefense: 25, attackSpeed: "0.86", attackRange: "3.30", normalAttackTargetCount: 2, description: "火炎を極めた果てに蒼黒の水流を操る上位竜。広い射程と重い奔流で戦線を支配する。", fusionExclusive: 0),
    ...
    MonsterEntry(raceFolder: "ゴーレム", sourceFolder: "ゴーレム3", resourceFolder: "Golem", outputKey: "cosmic_ore_fortress_golem", monsterId: "monster_cosmic_ore_fortress_golem", displayName: "宇宙鉱石要塞ゴーレム", raceId: "golem", classRank: 3, encyclopediaNumber: 9, element: 2, rangeType: 0, damageType: 0, maxHp: 184, attack: 35, magicAttack: 8, defense: 50, magicDefense: 28, attackSpeed: "0.62", attackRange: "1.15", normalAttackTargetCount: 1, description: "宇宙鉱石に蓄えた蒼い霊水で動く要塞級ゴーレム。圧倒的な耐久力を誇る。"),
    ...
    MonsterEntry(raceFolder: "4クラス", sourceFolder: "暗黒魔導機神メルキオン", resourceFolder: "Class4", outputKey: "dark_magic_machine_god_merchion", monsterId: "monster_dark_magic_machine_god_merchion", displayName: "暗黒魔導機神メルキオン", raceId: "special", classRank: 4, encyclopediaNumber: 22, element: 5, rangeType: 0, damageType: 1, maxHp: 178, attack: 22, magicAttack: 74, defense: 42, magicDefense: 48, attackSpeed: "0.74", attackRange: "1.24", normalAttackTargetCount: 2, description: "暗黒魔導を内蔵した機神。機械の巨体と魔力で前線を焼き払う。", fusionExclusive: 1),
...
func clearColumns(_ rep: NSBitmapImageRep, xRange: Range<Int>) {
    guard let data = rep.bitmapData else {
        return
    }
    ...
    let startX = max(0, min(width, xRange.lowerBound))
    let endX = max(startX, min(width, xRange.upperBound))
    for y in 0..<height {
...
func applyFrameCleanup(_ frame: NSBitmapImageRep, prefix: String, index: Int) {
    if prefix == "mon_abyss_grand_mage_seraphis_attack" && index == 3 {
        clearColumns(frame, xRange: 0..<38)
    }
    if prefix == "mon_spirit_queen_titania_attack" && index == 2 {
        clearColumns(frame, xRange: 0..<374)
    }
}
...
func detectCorrectedFrameCount(from sheet: NSBitmapImageRep, estimatedFrameCount: Int) -> Int {
    ...
    if width % 4 == 0 {
        let fourFrameWidth = width / 4
        let cellAspect = Double(fourFrameWidth) / Double(max(1, height))
        if cellAspect >= 0.6 && cellAspect <= 1.35 {
            return 4
        }
    }
...
func processSheet(source: String, outputDirectory: String, prefix: String) throws {
    ...
    if let contentFrames = extractContentBasedFrames(from: sheet, expectedFrameCount: frameCount) {
        for (index, frame) in contentFrames.enumerated() {
            applyFrameCleanup(frame, prefix: prefix, index: index)
...
func defaultBattleVisualScale(for entry: MonsterEntry) -> String {
    let overrides: [String: String] = [
        ...
        "monster_sword_saint_alvarez": "1.4",
        "monster_apprentice_mage": "0.88",
        "monster_dark_robe_curse_mage_noah": "0.96",
        "monster_abyss_grand_mage_seraphis": "1.4",
        ...
        "monster_spirit_queen_titania": "1.30"
    ]
